...
CScope::CScope()
{
	...
	m_strUnitName = _T("度");
	m_BkBrush.CreateSolidBrush(RGB(0x00,0x00,0x33));		//color of the top
	m_PanelBrush.CreateSolidBrush(RGB(0x22,0x22,0x44));		//color of panel
	m_CurveBrush.CreateSolidBrush(RGB(0x00,0x00,0x00));		//背景

					
	m_PenCurve.CreatePen(PS_SOLID, 2, RGB(style, width, color));														//线条颜色
	m_PenGrid.CreatePen(PS_SOLID,0,RGB(192,192,192));				//刻度
	m_PenBrightLine.CreatePen(PS_SOLID,0,RGB(0xff,0xff,0xff));		//外边框
	m_PenDarkLine.CreatePen(PS_SOLID,0,RGB(0x55,0x55,0x55));		//边框
...
void CScope::DrawCurve(CDC *pDC)
{
		...
		m_PenCurve.CreatePen(style, width,  color);
...
{
	m_strValueName = strTitle;		//名称
	m_strUnitName = strUnit;		//单位
	Invalidate();
}
void CScope::Clear()
{
